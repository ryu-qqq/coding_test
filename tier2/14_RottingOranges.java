 ...
 *   1) 모든 썩은 오렌지를 큐에 넣고, 신선 개수(fresh)를 카운트.
 *   2) 큐가 빌 때까지 분 단위(level)로 BFS:
 *        for each in 현재 큐 사이즈:
 *          꺼내서 4방향에 신선이 있으면 썩히고 큐에 추가, fresh--
 *        분 ++
 *   3) BFS 끝난 뒤 fresh > 0 이면 -1, 아니면 분 (시작 시점 신선이 없으면 0).
 *
 * --- 불변식 ---
